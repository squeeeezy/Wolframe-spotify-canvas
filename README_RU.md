...
> **Рабочий стандарт для получения Spotify Canvas (видео-лупов) в 2026 году.**
...
-   **Гибкость:** Создана с расчетом на будущие обновления. Вы можете переопределить хеш GraphQL запроса и имя операции через конфиг, если Spotify решит их сменить.
...
### "Ловушка Хэша" (Hash Trap)
GraphQL API Spotify не принимает "сырые" запросы. Он требует **Persisted Queries** — отправку SHA-256 хеша запроса.
*   **Хеш по умолчанию:** `575138ab27cd5c1b3e54da54d0a7cc8d85485402de26340c2145f0f6bb5e7a9f` (Зашит в либу, но меняется через конфиг).
*   **Operation Name:** `canvas` (Важно: раньше все думали, что это `getCanvas`, но это вызывало 400 ошибку).
*   **Имя переменной:** `trackUri` (Раньше было `uri`).

### Авторизация
Простого access токена **недостаточно**. API требует заголовок `client-token`.
*   Эта библиотека эмулирует флоу сайта `clienttoken.spotify.com`, генерируя случайный `device_id` и обменивая его на валидный временный `client-token`.

## 🤝 Вклад в проект (Contributing)

Пул-реквесты приветствуются! Особенно если Spotify обновит хеш API — PR с обновленным `DEFAULT_CANVAS_HASH` спасет тысячи разработчиков.
...
*Сделано с ❤️ от команды Wolframe.*
